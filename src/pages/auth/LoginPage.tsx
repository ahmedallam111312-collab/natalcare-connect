...
import { signInWithEmailAndPassword, sendPasswordResetEmail, signInAnonymously, signInWithPopup, GoogleAuthProvider } from "firebase/auth";
...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!userDoc.exists()) {
        // First time login via Google -> Default to patient
      ...
      if (error.code !== 'auth/popup-closed-by-user') {
        setErrorMsg("فشل تسجيل الدخول بواسطة Google. حاول مجدداً.");
      }
    } finally {
      setIsLoading(false);
    }
